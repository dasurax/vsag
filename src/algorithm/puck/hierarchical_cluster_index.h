...
#include "algorithm/puck/puck_data_pool.h"
#include "algorithm/puck/index.h"
#include "algorithm/puck/time.h"
#include "algorithm/puck/func_timer.h"
...
public:
    ...
    **/
    ...
    virtual IndexConf& get_conf_file() override {
        return _conf;
    }

    virtual void set_conf_file(IndexConf& conf) override {
        _conf = conf;
    }
protected:
    /*
    ...
    * 直接从文件读取数据初始化内存
    */
    virtual int init_model_memory(char * fileBaseAddress, uint64_t fileLength);
    /*
     * @brief 读码本文件
     * @@return (int) : 正常返回0，错误返回值<0
     **/
    virtual int read_codebooks();
    /*
     * @brief 码本数据直接从文件加载使用，这里补充部分需要在线计算的结果 
     * @@return (int) : 正常返回0，错误返回值<0
     **/
    virtual int fill_codebooks();
    /*
     * @brief 写码本文件
     * @@return (int) : 正常返回0，错误返回值<0
     **/
    virtual int save_codebooks() const;
    /*
     * @brief 写索引文件(建库的产出，与建库样本相关)
     * @@return (int) : 正常返回0，错误返回值<0
     **/
    virtual int save_index();
    virtual int save_index_bak();
    virtual int save_index_after_convert();
    /*
     ...
     **/
    virtual char* load_model_config(char* ptr, size_t total_size);
    /*
    ...
    virtual void batch_assign(const uint32_t total_cnt, const std::string& feature_file_name,
                              uint32_t* cell_assign, bool bak_assign = false);
    ...
    virtual int convert_local_to_memory_idx(
            ...
            bool need_init = false) {return 0;}
    /*
    * @brief 加载与样本相关的索引文件
    * @@param [in] local_to_memory_idx : 每个样本local_idx 与 memory_idx的映射关系
    * @@return (int) : 正常返回0，错误返回值<0
    **/
    virtual int read_feature_index(uint32_t* local_to_memory_idx, bool reset_quantized_feature = true);

    virtual int init_on_brutal_serving() { return 0; }

    /*
    * 直接加载原始特征使用文件内容
    **/
    int read_src_feature_index(char * fileBaseAddress, uint64_t fileLength);
    ...
    **/
    int assign(const ThreadParams& thread_params, uint32_t* cell_assign, float* error_distance,
               float* pruning_computation, bool bak_assign = false, uint32_t offset = 0) const;
    /*
    ...
    **/
    ...
    int bak_cell_assign(const float* coarse_distance,
                            const float* fine_distance,
                            const float query_norm,
                            NearestCell& nearest_cell,
                            uint32_t gt_cell_id,
    ...
    int bak_cell_assign2(const float* coarse_distance,
                            const float* fine_distance,
                            const float query_norm,
                            NearestCell& nearest_cell,
                            uint32_t gt_cell_id,
    ...
    void scalar_quantize_float_dataset(
                            const uint32_t total_cnt, 
                            const std::string& feature_file_name);

    int compute_max_quantization_multipliers_and_norms(
                            const ThreadParams& thread_params,
                            std::vector<float>& max_quantization_multipliers,
                            std::vector<float>& dp_norms) const;

    int scalar_quantize_float_dataset_with_multipliers(
                            const ThreadParams& thread_params, 
...
protected:
    ...
    char* _model;
    uint32_t* _cell_assign;
    uint32_t* _memory_to_local;
    uint32_t* _local_to_memory;
    uint32_t* _memory_to_ori_cell_id;
    float*  _all_feature;
    int8_t*  _all_feature_int8;
    float*  _int8_multipliers;
    float*  _dp_norms;
    bool _converted = false;
    float* _coarse_inner_products;
    float* _fine_inner_products;
    float* _coarse_fine_products;
...
struct BuildInfo {
    ...
    std::vector<NearestCell> nearest_cell;
    std::vector<int> nearest_cell_id;
    virtual ~BuildInfo() {}

    int getSize() {
        int size = lable.size() + feature.size() * 4 + other_str_data.size() + nearest_cell.size() * sizeof(NearestCell);
        size += sizeof(int) * nearest_cell_id.size();
        return size;
    }
};
