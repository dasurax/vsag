...
struct KmeansParams {
    ...
    KmeansParams(bool kmeans_pp = false, int threads_count = std::thread::hardware_concurrency()) {
        redo    = 1;
        nt      = threads_count;
        niter   = 30;
...
public:
    Kmeans(): _rnd(time(0)) {}
    Kmeans(bool kmeans_pp, int threads_count = std::thread::hardware_concurrency()): _params(kmeans_pp, threads_count), _rnd(time(0)) {}
    ~Kmeans() {}
    float kmeans(uint32_t dim, size_t n, size_t k,
                 const float* v,
                 float* centroids_out, float* dis_out = nullptr,
                 int* assign_out = nullptr);
    float assign(uint32_t dim, size_t n, size_t k,
                 const float* v,
                 float* centroids_out, float* dis_out = nullptr,
                 int* assign_out = nullptr);
...
protected:
    ...
    int kmeans_core(uint32_t dim, size_t total_cnt, size_t k, int niter, int nt,
                    ...
                    float* dis,
                    double* qerr_out);
    int kmeans_assign(uint32_t dim, size_t total_cnt, size_t k, int niter, int nt,
                    float* centroids, const float* v,
                    int* assign, int* nassign,
                    float* dis,
                    double* qerr_out);
private:
    ...
    inline int nearest_center(uint32_t dim, const float* centroids, const size_t centroid_cnt,
