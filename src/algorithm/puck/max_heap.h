...
class PuckMaxHeap {
public:
    PuckMaxHeap(const uint32_t size, float* val, uint32_t* tag);
    PuckMaxHeap(const uint32_t size, float* val, float* val_aux, uint32_t* tag);
    ~PuckMaxHeap() {}
    /*
     * @brief 小于堆顶元素时，需要更新堆
     * @@param [in] new_val : 距离
    ...
    void max_heap_update(const float new_val, const float new_val_aux, const uint32_t new_tag);
    /*
     * @brief 排序
     **/
    void reorder();
...
private:
    ...
    void insert(uint32_t heap_size, uint32_t father_idx, float  new_val, float new_val_aux, uint32_t new_tag);
