...
IndexConf::IndexConf() {
    ...
    filter_ks = FLAGS_filter_ks;
    use_lut16 = filter_ks == 16;
    ip2cos = FLAGS_ip2cos;
    ...
    if (gflags::GetCommandLineFlagInfo("nsq", &info) && info.is_default) {
        std::string feature_dim_str = std::to_string(feature_dim);
        gflags::SetCommandLineOption("nsq", feature_dim_str.c_str());
    ...
    search_fine_count = FLAGS_search_fine_count;
    ...
    sparse_feature_file_name = index_path + "/" + FLAGS_sparse_feature_file_name;

    if (FLAGS_feature_file_name_udf != "") {
        feature_file_name = FLAGS_feature_file_name_udf;
    }
    //LOG(INFO) << "feature_file_name: " << feature_file_name << " " << index_path;

    int8_dataset_file_name = index_path + "/int8_dataset.dat";
    int8_multipliers_file_name = index_path + "/int8_multipliers.dat";
    dp_norms_file_name = index_path + "/dp_norms.dat";
    ...
    cell_start_memory_idx_file_name = index_path + "/" + FLAGS_cell_start_memory_idx_file_name;
    cell_assign_bak_file_name = index_path + "/" + FLAGS_cell_assign_bak_file_name;

    pq_codebook_file_name = index_path + "/" + FLAGS_pq_codebook_file_name;
    label_file_name = index_path + "/" + FLAGS_label_file_name;

    local_to_memory_file_name = index_path + "/" + FLAGS_local_to_memory_file_name;
    memory_to_local_file_name = index_path + "/" + FLAGS_memory_to_local_file_name;
    memory_to_ori_cell_id_file_name = index_path + "/" + FLAGS_memory_to_ori_cell_id_file_name;
    ...
    filter_data_bak_file_name = index_path + "/" + FLAGS_filter_data_bak_file_name;
    ...
    hierarchical_cluster_index = index_path + "/" + FLAGS_hierarchical_cluster_index_data_file_name;

    spilled_copy_num = 1;
    spilled_coarse_sample_factor = 20;
    spilled_coarse_min_sample_num = 20;
    if (FLAGS_index_type_ad_hoc == 2) {
        spilled_copy_num = 0;
    }
...
int IndexConf::adaptive_train_param() {
    ...
    if (index_type == IndexType::PUCK) {
        ...
        //filter_nsq 未设置
        whether_filter = true;
        bool unset_filter_nsq = gflags::GetCommandLineFlagInfo("filter_nsq", &info) && info.is_default;
        ...
        if (unset_filter_nsq && ip2cos == 0) {
            ...
            do {
                if (feature_dim % lsq == 0) {
                    ...
                    LOG(INFO) << "show conf after adaptive_train_param";
                    show();
                    return 0;
                }
...
int IndexConf::adaptive_search_param() {
    ...
    if (index_type == IndexType::TINKER) {
        ...
        if (gflags::GetCommandLineFlagInfo("tinker_search_range", &info) && info.is_default) {
            tinker_search_range = 5 * topk;
        }

        //Tinker的检索一级聚类中心的个数很少，20足矣
        if (gflags::GetCommandLineFlagInfo("search_coarse_count", &info) && info.is_default) {
    ...
    if (index_type == IndexType::HIERARCHICAL_CLUSTER) {
        ...
        gflags::CommandLineFlagInfo info;
        //filter_topk = 20;
        LOG(INFO) << "adaptive_search_param: change filter_topk( " << filter_topk << " ) topk( " << topk << " )";
        //如果没有设置filter_topk，更新filter_topk，取值与topk有关
        if (gflags::GetCommandLineFlagInfo("filter_topk", &info) && info.is_default) {
            if(filter_topk < topk) {
                LOG(INFO) << "adaptive_search_param: change filter_topk( " << filter_topk << " ) topk( " << topk << " )";
                filter_topk = topk * 11;
            }
...
void IndexConf::show() {
    ...
    LOG(INFO) << "search_fine_count = " << search_fine_count;
    ...
    LOG(INFO) << "use_lut16 = " << use_lut16;
    LOG(INFO) << "spilled_copy_num = " << spilled_copy_num;
    LOG(INFO) << "brutal_search = " << brutal_search;
    LOG(INFO) << "freshness_test = " << freshness_test;
}
