...
DEFINE_bool(need_log_file, true, "need log file");
DEFINE_string(puck_log_file, "puck.log", "log file name");
...
DEFINE_string(sparse_feature_file_name, "all_data.csr.bin", "feature data of all points");
DEFINE_string(feature_file_name_udf, "", "feature data of all points");
...
DEFINE_string(cell_start_memory_idx_file_name, "cell_start_memory_idx.dat", "cell assign");
DEFINE_string(cell_assign_bak_file_name, "cell_assign_bak.dat", "cell assign bak");
//realtime insert和分布式建库的索引需要这个索引文件，search返回的local id是这个样本在该文件的行数
DEFINE_string(label_file_name, "all_data.url", "label of points");


DEFINE_string(local_to_memory_file_name, "local_to_memory.dat", "local_to_memory_file_name");
DEFINE_string(memory_to_local_file_name, "memory_to_local.dat", "memory_to_local_file_name");
DEFINE_string(memory_to_ori_cell_id_file_name, "memory_to_ori_cell_id.dat", "memory_to_ori_cell_id_file_name");
...
DEFINE_string(hierarchical_cluster_index_data_file_name, "hierarchical_cluster_index.dat", "hierarchical_cluster_index data");
...
DEFINE_string(filter_data_bak_file_name, "filter_data_bak.dat", "filter data of points");
...
DEFINE_bool(use_lut16, false, "");
DEFINE_bool(use_int8, false, "");
DEFINE_bool(use_fast_heap, false, "");

DEFINE_int32(filter_ks, 16, "");

DEFINE_bool(only_train_pq, false, "");
DEFINE_int32(search_fine_count, 10, "");
DEFINE_int32(search_point_count, 0, "");

DEFINE_bool(packed_cluster, false, "");
DEFINE_bool(enable_lut_clip, false, "");
DEFINE_int32(window_size, 100, "");

DEFINE_bool(use_pq_method_0, true, "");
DEFINE_bool(use_pq_method_1, false, "");
DEFINE_bool(use_avg_offset, false, "");
DEFINE_bool(use_bak_cluster, true, "");


DEFINE_bool(convert_local_to_memory_while_build, true, "");

DEFINE_int32(bak_cell_assign, 2, "");

//DEFINE_bool(ad_hoc, false, "if train/build without aitheta");
DEFINE_bool(ad_hoc, true, "if train/build without aitheta");

DEFINE_bool(only_build, true, "");
DEFINE_int32(index_type_ad_hoc, 1, "");

DEFINE_bool(use_brute_force, false, "");

DEFINE_bool(spilled_assgin_in_ip_space, false, "");
DEFINE_bool(only_sample, false, "");

DEFINE_int32(sq_num_bits, -1, "");
