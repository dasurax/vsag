...
long
readProcStatus2(const std::string& name) {
...
DEFINE_int32(single_build_max_points,
             1,
             "during single build process, max points can be saved in memory");

void
matrix_multiplication(const float* left,
                      const float* right,
                      FINTEGER m,
                      FINTEGER n,
                      FINTEGER k,
...
//检查文件长度
bool
check_file_length_info(const std::string& file_name, const uint64_t file_length) {
    int fd = -1;
...
//写码本
int
write_fvec_format(const char* file_name,
                  const uint32_t dim,
                  const uint64_t n,
                  const float* fea_vocab) {
...
int
read_fvec_format(FILE* fvec_init, uint32_t dim, uint64_t n, float* v) {
    for (uint64_t i = 0; i < n; ++i) {
        int cur_dim = 0;
...
int
read_fvec_format(const char* file_name, uint32_t dim, uint64_t n, float* v) {
...
void
HierarchicalClusterIndex::init_params_value() {
...
int
HierarchicalClusterIndex::read_codebooks() {
    LOG(INFO) << "start load index file " << _conf.coarse_codebook_file_name;
    //一级聚类中心文件长度检查
    uint64_t coarse_vocab_length =
        (uint64_t)_conf.coarse_cluster_count * _conf.feature_dim * sizeof(float);
    uint64_t expect_length =
        (uint64_t)_conf.coarse_cluster_count * sizeof(int) + coarse_vocab_length;
    ...
    int ret = read_fvec_format(_conf.coarse_codebook_file_name.c_str(),
                               _conf.feature_dim,
                               _conf.coarse_cluster_count,
                               _coarse_vocab);
    if (ret != (int)_conf.coarse_cluster_count) {
        LOG(FATAL) << "load file error, file : " << _conf.coarse_codebook_file_name
                   << " feature_dim : " << _conf.feature_dim
                   << " number : " << _conf.coarse_cluster_count << " return code : " << ret;
        return -2;
    }

    for (uint32_t i = 0; i < _conf.coarse_cluster_count; ++i) {
        _coarse_norms[i] = cblas_sdot(_conf.feature_dim,
                                      _coarse_vocab + _conf.feature_dim * i,
                                      1,
                                      _coarse_vocab + _conf.feature_dim * i,
                                      1) /
                           2;
    ...
    //二级聚类中心文件长度检查
    uint64_t fine_vocab_length =
        (uint64_t)_conf.fine_cluster_count * _conf.feature_dim * sizeof(float);
    ...
    ret = read_fvec_format(_conf.fine_codebook_file_name.c_str(),
                           _conf.feature_dim,
                           _conf.fine_cluster_count,
                           _fine_vocab);

    if (ret != (int)_conf.fine_cluster_count) {
        LOG(FATAL) << "load file error, file : " << _conf.fine_codebook_file_name
                   << " feature_dim : " << _conf.feature_dim
                   << " number : " << _conf.coarse_cluster_count << " return code : " << ret;
        return -3;
    }
    ...
    for (uint32_t i = 0; i < _conf.fine_cluster_count; ++i) {
        fine_norms[i] = cblas_sdot(_conf.feature_dim,
    ...
    //矩阵乘
    matrix_multiplication(_fine_vocab,
                          _coarse_vocab,
                          _conf.fine_cluster_count,
                          _conf.coarse_cluster_count,
                          _conf.feature_dim,
    ...
    if (_conf.spilled_copy_num > 0) {
        // CC
        matrix_multiplication(_coarse_vocab,
                              _coarse_vocab,
                              _conf.coarse_cluster_count,
                              _conf.coarse_cluster_count,
                              _conf.feature_dim,
                              "TN",
                              _coarse_inner_products);
...
int
HierarchicalClusterIndex::fill_codebooks() {
    for (uint32_t i = 0; i < _conf.coarse_cluster_count; ++i) {
        _coarse_norms[i] = cblas_sdot(_conf.feature_dim,
                                      _coarse_vocab + _conf.feature_dim * i,
                                      1,
...
int
HierarchicalClusterIndex::save_codebooks() const {
...
int
HierarchicalClusterIndex::read_index() {
    ...
    if (FLAGS_convert_local_to_memory_while_build) {
        ...
        if (convert_local_to_memory_idx(cell_start_memory_idx.data(), local_to_memory_idx.data()) !=
            0) {
            return -1;
        }
...
int
HierarchicalClusterIndex::read_src_feature_index(char* fileBaseAddress, uint64_t fileLength) {
    // 原始特征文件长度检查
    u_int64_t all_feature_length =
        (u_int64_t)_conf.total_point_count * _conf.feature_dim * sizeof(float);
    if (all_feature_length != fileLength) {
        LOG(ERROR) << "Unexpected source feature length: " << fileLength
                   << ", expected: " << all_feature_length << ".";
        return -1;
    }
...
int
HierarchicalClusterIndex::read_feature_index(uint32_t* local_to_memory_idx,
                                             bool reset_quantized_feature) {
    LOG(INFO) << "start load index file " << _conf.feature_file_name << " " << FLAGS_whether_pq
              << " " << FLAGS_use_int8;
    ...
    if (false) {
        ...
        u_int64_t all_feature_length =
            (u_int64_t)_conf.total_point_count * _conf.feature_dim * sizeof(float);
        u_int64_t expect_length =
            (u_int64_t)_conf.total_point_count * sizeof(int) + all_feature_length;
        ...
        for (uint32_t idx = 0; idx < _conf.total_point_count; ++idx) {
            uint64_t memory_offset =
                (u_int64_t)local_to_memory_idx[idx] * (u_int64_t)_conf.feature_dim;
            read_fvec_format(
                init_feature_stream, _conf.feature_dim, 1, _all_feature + memory_offset);
        }
    ...
    if (!FLAGS_use_int8) {
        ...
        //原始特征文件长度检查
        u_int64_t int8_dataset_length =
            (u_int64_t)_conf.total_point_count * _conf.feature_dim * sizeof(int8_t);
        ...
        u_int64_t all_feature_length =
            int8_dataset_length + int8_multipliers_length + dp_norms_length;
        ...
        is_ok = is_ok &&
                check_file_length_info(_conf.int8_multipliers_file_name, int8_multipliers_length);
...
void
load_int_array_by_idx_thread_fun(const std::string& file_name,
                                 const int start_id,
                                 const int count,
                                 uint32_t* point_id) {
...
int
HierarchicalClusterIndex::load_cell_assign(uint32_t* cell_assign) {
    ...
    all_cost.stop();
    LOG(INFO) << "load index file " << cell_assign_file_name << " cost " << all_cost.m_elapsed()
              << " ms";
    return 0;
}

int
HierarchicalClusterIndex::load_int_array_by_idx(uint32_t* memory,
    ...
    for (uint32_t thread_id = 0; thread_id < _conf.threads_count; ++thread_id) {
        ...
        threads.push_back(std::thread(
            std::bind(load_int_array_by_idx_thread_fun, file_name, start_id, count, memory)));
    }
...
int
HierarchicalClusterIndex::convert_local_to_memory_idx(uint32_t* cell_start_memory_idx,
    ...
    for (uint32_t i = 0; i < _conf.fine_cluster_count * _conf.coarse_cluster_count; ++i) {
        ...
        _coarse_clusters[coarse_id].min_dist_offset = std::min(
            _coarse_clusters[coarse_id].min_dist_offset, cur_fine_cluster.stationary_cell_dist);
    }
...
template <typename T>
char*
SetValueAndIncPtr(char* ptr, const T& val) {
    *((T*)(ptr)) = val;
    return ptr + sizeof(T);
}

size_t
HierarchicalClusterIndex::save_model_config(char* ptr) const {
    char* temp_ptr = ptr;
    //索引数据相关
...
template <typename T>
char*
GetValueAndIncPtr(char* ptr, T& val) {
    val = *((T*)(ptr));
    return ptr + sizeof(T);
}

char*
HierarchicalClusterIndex::load_model_config(char* ptr, size_t total_size) {
...
int
HierarchicalClusterIndex::init_model_memory() {
    TIME_FUNCTION();
    if (_model != nullptr) {
        free(_model);
        _model = nullptr;
    ...
    size_t model_len = 0;
    size_t coarse_vocab_length =
        (size_t)_conf.coarse_cluster_count * _conf.feature_dim * sizeof(float);
    ...
    LOG(INFO) << "coarse_clusters_length " << coarse_clusters_length << " " << sizeof(CoarseCluster)
              << " " << _conf.coarse_cluster_count;
    uint32_t cell_cnt = _conf.coarse_cluster_count * _conf.fine_cluster_count;
...
int
HierarchicalClusterIndex::init_model_memory(char* fileBaseAddress, uint64_t fileLength) {
    TIME_FUNCTION();
    size_t model_len = 0;
    size_t coarse_vocab_length =
        (size_t)_conf.coarse_cluster_count * _conf.feature_dim * sizeof(float);
...
int
HierarchicalClusterIndex::read_model_file() {
    int fd = -1;
    ...
    LOG(INFO) << "part_size=" << part_size << " st.st_size= " << st.st_size
...
int
HierarchicalClusterIndex::check_index_type() {
...
int
HierarchicalClusterIndex::save_model_file() const {
    /// index_conf
...
int
HierarchicalClusterIndex::search_nearest_coarse_cluster(SearchContext* context,
                                                        const float* feature,
...
int
HierarchicalClusterIndex::search_nearest_fine_cluster(SearchContext* context,
                                                      const float* feature) {
    //base::Timer tm_cost;
    //tm_cost.start();
    SearchCellData& search_cell_data = context->get_search_cell_data();
    ...
    for (uint32_t l = 0; l < _conf.search_coarse_count; ++l) {
        ...
        float max_stationary_dist =
            imitative_heap.get_pivot() - coarse_distance[l] - search_cell_data.fine_distance[0];
        ...
        for (uint32_t idx = 0; idx < _conf.fine_cluster_count; ++idx) {
            ...
            int ret = imitative_heap.push(
                temp_dist, cur_fine_cluster_list + k, (cur_fine_cluster_list + k)->get_point_cnt());

            if (ret == 0) {
                max_stationary_dist = imitative_heap.get_pivot() - coarse_distance[l] -
                                      search_cell_data.fine_distance[idx];
            }
        }
    }
    ...
    std::sort(search_cell_data.cell_distance.begin(),
              search_cell_data.cell_distance.begin() + cell_cnt);
    return cell_cnt;
}

int
HierarchicalClusterIndex::search(const Request* request, Response* response) {
...
int
HierarchicalClusterIndex::compute_exhaustive_distance_with_points(SearchContext* context,
                                                                  const int cell_idx,
                                                                  const float* feature,
    ...
    for (uint32_t i = 0; i < point_list_cnt; ++i) {
        const float* exhaustive_feature =
            _all_feature + (cur_fine_cluster->memory_idx_start + i) * _conf.feature_dim;
        float temp_dist = 0;
        ...
        for (; (m + qty_16) <= _conf.feature_dim && temp_dist < result_distance[0];) {
            temp_dist +=
                similarity::L2Sqr16Ext(exhaustive_feature + m, feature + m, qty_16, TmpRes);
            m += qty_16;
        }
        ...
        if (temp_dist < result_distance[0]) {
            ...
            if (left_dim > 0) {
                temp_dist +=
                    similarity::L2SqrExt(exhaustive_feature + m, feature + m, left_dim, TmpRes);
            }

            result_heap.max_heap_update(temp_dist,
                                        _memory_to_local[cur_fine_cluster->memory_idx_start + i]);
        }
    }
...
int
HierarchicalClusterIndex::flat_topN_points(SearchContext* context,
                                           const float* feature,
...
int
HierarchicalClusterIndex::train(const u_int64_t kmenas_point_cnt, float* kmeans_train_vocab) {
    TIME_FUNCTION();
    base::Timer tm_cost;
    tm_cost.start();
    ...
    memcpy(train_vocab.get(),
           kmeans_train_vocab,
           sizeof(float) * kmenas_point_cnt * _conf.feature_dim);
    //记录每次kmeans产生的聚类中心
    std::unique_ptr<float[]> coarse_init_vocab(
        new float[_conf.coarse_cluster_count * _conf.feature_dim]);
    //std::unique_ptr<int[]> coarse_vocab_assign(new int[kmenas_point_cnt]);
    std::unique_ptr<float[]> fine_init_vocab(
        new float[_conf.fine_cluster_count * _conf.feature_dim]);
    ...
    for (int ite = 0; ite < FLAGS_kmeans_iterations_count; ++ite) {
        ...
        float err = kmeans_cluster.kmeans(_conf.feature_dim,
                                          kmenas_point_cnt,
                                          _conf.coarse_cluster_count,
                                          train_vocab.get(),
                                          coarse_init_vocab.get(),
                                          nullptr,
                                          cluster_assign.get());
        ...
        for (uint32_t i = 0; i < kmenas_point_cnt; ++i) {
            ...
            cblas_saxpy(_conf.feature_dim,
                        -1.0,
                        coarse_init_vocab.get() + assign_id * _conf.feature_dim,
                        1,
                        train_vocab.get() + i * _conf.feature_dim,
                        1);
        }
        ...
        // 500000
        err = kmeans_cluster.kmeans(_conf.feature_dim,
                                    kmenas_point_cnt,
                                    _conf.fine_cluster_count,
                                    train_vocab.get(),
                                    fine_init_vocab.get(),
                                    nullptr,
                                    cluster_assign.get());
        LOG(INFO) << ite << " deviation error of init fine clusters is " << err
                  << " when ite = " << ite;
        ...
        if ((min_err - err) >= 1e-4) {
            ...
            LOG(INFO) << "current deviation error > min deviation error : " << err << " / "
                      << min_err << ", params.niter = " << params.niter;
        ...
        for (uint32_t i = 0; i < kmenas_point_cnt; ++i) {
            ...
            cblas_saxpy(_conf.feature_dim,
                        -1.0,
                        _fine_vocab + assign_id * _conf.feature_dim,
                        1,
                        train_vocab.get() + i * _conf.feature_dim,
                        1);
        }
    ...
    PuckMaxHeap max_heap(
        _conf.fine_cluster_count, fine_reorder_dist.data(), fine_reorder_id.data());
    ...
    for (uint32_t j = 0; j < _conf.fine_cluster_count; ++j) {
        ...
        float temp = cblas_sdot(_conf.feature_dim,
                                _fine_vocab + _conf.feature_dim * j,
                                1,
                                _fine_vocab + _conf.feature_dim * j,
                                1) /
                     2;
...
int
HierarchicalClusterIndex::build() {
    TIME_FUNCTION();
    if (_conf.brutal_search) {
        return 0;
    }
    ...
    if (_conf.spilled_copy_num > 0) {
        batch_assign(_conf.total_point_count,
                     _conf.feature_file_name,
                     cell_assign + _conf.total_point_count,
                     true);
    ...
    std::vector<uint32_t> cell_start_memory_idx(cell_cnt + 1, total_point_count);
    std::vector<uint32_t> local_to_memory_idx(total_point_count);
    LOG(INFO) << "scalar_quantize_float_dataset: " << _conf.total_point_count << " "
              << _conf.feature_file_name;
    ...
    if (FLAGS_convert_local_to_memory_while_build && FLAGS_ad_hoc) {
        ...
        if (convert_local_to_memory_idx(
                cell_start_memory_idx.data(), local_to_memory_idx.data(), cell_assign) != 0) {
            return -1;
        }
        ...
        long ret =
            fwrite(cell_start_memory_idx.data(), sizeof(cell_start_memory_idx[0]), cell_cnt + 1, f);
        fclose(f);
...
int
HierarchicalClusterIndex::save_index() {
    TIME_FUNCTION();
    save_model_file();
    ...
    long ret = fwrite(_cell_assign,
                      sizeof(*_cell_assign),
                      _conf.total_point_count * (1 + _conf.spilled_copy_num),
                      f);
...
int
HierarchicalClusterIndex::save_index_after_convert() {
    FILE* f = fopen(_conf.cell_assign_file_name.c_str(), "wb");
...
//point距离最近的cell信息
int
HierarchicalClusterIndex::nearest_cell_assign(const float* coarse_distance,
    ...
    for (uint32_t i = 0; i < _conf.coarse_cluster_count; ++i) {
        ...
        for (uint32_t idx = 0; idx < _conf.fine_cluster_count; ++idx) {
            ...
            //}
            if (std::sqrt(nearest_cell.distance) - _fine_norms[idx] <
                std::sqrt(current_coarse_term)) {
                //记录跳过的cell个数占比
            ...
            float score = current_coarse_term - fine_distance[cur_fine_id] +
...
float
cosine(float* a, float* b, int dim) {
    // a * b / |a||b|
...
int
HierarchicalClusterIndex::bak_cell_assign2(const float* coarse_distance,
                                           const float* fine_distance,
                                           const float query_norm,
                                           NearestCell& nearest_cell,
                                           uint32_t gt_cell_id,
    ...
    for (uint32_t ii = 0; ii < coarse_sample_num; ++ii) {
        ...
        for (uint32_t idx = 0; idx < _conf.fine_cluster_count; ++idx) {
            ...
            float half_rnorm_sq =
                current_coarse_term  //_coarse_norms[i] - coarse_distance[i] + query_norm
...
int
HierarchicalClusterIndex::bak_cell_assign(const float* coarse_distance,
                                          const float* fine_distance,
                                          const float query_norm,
                                          NearestCell& nearest_cell,
                                          uint32_t gt_cell_id,
...
int
HierarchicalClusterIndex::assign(const ThreadParams& thread_params,
                                 uint32_t* cell_assign,
                                 float* error_distance,
                                 float* pruning_computation,
                                 bool bak_assign,
                                 uint32_t offset) const {
    std::unique_ptr<float[]> points_coarse_terms(
        new float[FLAGS_thread_chunk_size * _conf.coarse_cluster_count]);
    std::unique_ptr<float[]> points_fine_terms(
        new float[FLAGS_thread_chunk_size * _conf.fine_cluster_count]);
    ...
    for (uint32_t cid = 0; cid < thread_params.chunks_count; ++cid) {
        ...
        int real_thread_chunk_size =
            std::min(FLAGS_thread_chunk_size,
                     (int)(thread_params.points_count - cid * FLAGS_thread_chunk_size));
        int read_chunk_size = read_fvec_format(thread_params.learn_stream,
                                               _conf.feature_dim,
                                               real_thread_chunk_size,
                                               chunk_points.get());

        if (read_chunk_size != real_thread_chunk_size) {
            LOG(ERROR) << "puck assign from " << thread_params.start_id
                       << " read file error at cid = " << cid << " / " << thread_params.chunks_count
                       << ", ret = " << read_chunk_size;
        ...
        for (int point_id = 0; point_id < real_thread_chunk_size; ++point_id) {
            ...
            float* cur_coarse_distance =
                points_coarse_terms.get() + point_id * _conf.coarse_cluster_count;
            float* cur_fine_distance =
                points_fine_terms.get() + point_id * _conf.fine_cluster_count;
            float point_norms =
                cblas_sdot(_conf.feature_dim, cur_point_fea, 1, cur_point_fea, 1) / 2;
            ...
            if (bak_assign) {
                ...
                if (FLAGS_bak_cell_assign == 1) {
                    bak_cell_assign(
                        ...
                        nearest_cell,
                        (cell_assign -
                         offset)[thread_params.start_id + cid * FLAGS_thread_chunk_size + point_id],
            ...
            if (thread_params.start_id + cid * FLAGS_thread_chunk_size + point_id == 6395337) {
                LOG(INFO) << "nearest_cell.distance : " << nearest_cell.distance << " "
                          << nearest_cell.cell_id
                          << " s:" << nearest_cell.cell_id / _conf.fine_cluster_count
                          << " t:" << nearest_cell.cell_id % _conf.fine_cluster_count << " qs:"
                          << cur_coarse_distance[nearest_cell.cell_id / _conf.fine_cluster_count]
                          << " qt:"
                          << cur_fine_distance[nearest_cell.cell_id % _conf.fine_cluster_count]
                          << " q^2:" << point_norms << " s^2:"
                          << _coarse_norms[nearest_cell.cell_id / _conf.fine_cluster_count]
                          << " t^2:" << _fine_norms[nearest_cell.cell_id % _conf.fine_cluster_count]
                          << " station:"
                          << get_fine_cluster(nearest_cell.cell_id)->stationary_cell_dist
                          << " cur_point_fea:" << cur_point_fea[0] << " " << cur_point_fea[1] << " "
                          << cur_point_fea[767];
            }
            ...
            *pruning_computation +=
                nearest_cell.pruning_computation * 1.0 / _conf.coarse_cluster_count;
        }
    }
...
//打开文件
int
ThreadParams::open_file(const char* train_fea_file_name, uint32_t feature_dim) {
    close_file();
    ...
    u_int64_t offset =
        (u_int64_t)start_id * feature_dim * sizeof(float) + (u_int64_t)start_id * sizeof(int);
    ...
    if (ret != 0) {
        ...
        LOG(FATAL) << "seek file " << train_fea_file_name << " error, need offset = " << offset
                   << " cur pos = " << ps.__pos
...
void
HierarchicalClusterIndex::batch_assign(const uint32_t total_cnt,
    ...
    if (_fine_norms == nullptr) {
        ...
        for (uint32_t j = 0; j < _conf.fine_cluster_count; ++j) {
            ...
            _fine_norms[j] = cblas_sdot(_conf.feature_dim,
                                        _fine_vocab + _conf.feature_dim * j,
                                        1,
                                        _fine_vocab + _conf.feature_dim * j,
                                        1) /
        ...
        threads.push_back(std::thread([&, threadId] {
            {
                ...
                thread_params.points_count =
                    std::min(thread_params.points_count, (int)(total_cnt - thread_params.start_id));

                if (thread_params.points_count > 0) {
                    try {
                        thread_params.chunks_count =
                            std::ceil(1.0 * thread_params.points_count / FLAGS_thread_chunk_size);
                        int param_stat =
                        ...
                        LOG(INFO) << "assign, thread_params.start_id = " << thread_params.start_id
                                  << " points_count = " << thread_params.points_count
                                  << " feature_file_name = " << feature_file_name
                                  << " threadId = " << threadId;
                        assign(thread_params,
                               cell_assign,
                               error_distance.data() + threadId,
                               pruning_computation.data() + threadId,
                               bak_assign,
                               total_cnt);
                    } catch (...) {
    ...
    delete[] _fine_norms;
    _fine_norms = nullptr;
    LOG(INFO) << "batch_assign succeeded, deviation error = " << total_error / total_cnt
              << ", pruning computation = " << total_pruning_computation / total_cnt;
...
void
HierarchicalClusterIndex::batch_assign_bak(const uint32_t total_cnt,
                                           const std::string& feature_file_name,
                                           uint32_t* cell_assign) {
...
int
HierarchicalClusterIndex::compute_max_quantization_multipliers_and_norms(
    ...
    for (uint32_t cid = 0; cid < thread_params.chunks_count; ++cid) {
        ...
        for (int point_id = 0; point_id < real_thread_chunk_size; ++point_id) {
            ...
            dp_norms[thread_params.start_id + cid * FLAGS_thread_chunk_size + point_id] =
                point_norms;
...
int
HierarchicalClusterIndex::scalar_quantize_float_dataset_with_multipliers(
    const ThreadParams& thread_params,
    std::vector<float>& multipliers,
    ...
    for (uint32_t cid = 0; cid < thread_params.chunks_count; ++cid) {
        ...
        for (int point_id = 0; point_id < real_thread_chunk_size; ++point_id) {
            float* cur_point_fea = chunk_points.get() + point_id * _conf.feature_dim;
            auto global_point_id =
                thread_params.start_id + cid * FLAGS_thread_chunk_size + point_id;
            int8_t* cur_quantized_dp =
                quantized_dp.data() + (size_t)global_point_id * _conf.feature_dim;

            float point_norms = cblas_sdot(_conf.feature_dim, cur_point_fea, 1, cur_point_fea, 1);
            for (size_t j = 0; j < _conf.feature_dim; j++) {
                cur_quantized_dp[j] = int8_quantize(cur_point_fea[j] * multipliers[j]);
            }
        }
    }
...
void
HierarchicalClusterIndex::scalar_quantize_float_dataset(const uint32_t total_cnt,
                                                        const std::string& feature_file_name) {
    ...
    */
    ...
    LOG(WARNING) << "quantized_dp: " << (int)quantized_dp[0] << " "
                 << (int)quantized_dp[0];  // << " " << (int)quantized_dp[6713829 * 1024 + 1];
    LOG(WARNING) << "quantized_dp: " << quantized_dp[0] / multipliers[0] << " "
                 << quantized_dp[0] / multipliers[1];
    LOG(WARNING) << "quantized_dp: " << quantized_dp.size() << " "
                 << (size_t)_conf.feature_dim * _conf.total_point_count << " " << _conf.feature_dim
...
///随机采样，去重
int
random_sampling(const std::string& init_file_name,
                const u_int64_t total_cnt,
                const u_int64_t sampling_cnt,
                const uint32_t feature_dim,
                float* sampling_vocab) {
    if (!sampling_vocab) {
    ...
    while (available_sample < sampling_cnt && filtered_cnt < total_cnt) {
        ...
        while (sampled_points.size() + available_sample < sampling_cnt &&
               filtered_cnt < total_cnt) {
        ...
        for (uint32_t i = 0; i < sampled_points.size(); ++i) {
            ...
            u_int64_t offset = (u_int64_t)true_point_idx * feature_dim * sizeof(float) +
                               (u_int64_t)true_point_idx * sizeof(int);
            ...
            if (cur_dim != feature_dim) {
                LOG(FATAL) << true_point_idx << " feature dim error, " << cur_dim
                           << " != " << feature_dim << init_file_name;
                learn_stream.close();
                return -1;
            }
            ...
            //重复数据检查
            base::MD5Sum(
                (void*)(sampling_vocab + vocab_offset), sizeof(float) * feature_dim, &md5_digest);
            ...
            if (available_sample % 100000 == 0) {
                LOG(INFO) << "read sample " << available_sample << " from source file "
                          << init_file_name;
            }
        }
    }
...
int
HierarchicalClusterIndex::check_feature_dim() {
    int fd = -1;
...
int
HierarchicalClusterIndex::train() {
    if (check_feature_dim() != 0) {
    ...
    if (_conf.train_points_count > 0) {
        ...
        LOG(INFO) << "HierarchicalClusterIndex set train_points_count to "
                  << _conf.train_points_count;
    }
    ...
    if (_conf.pq_train_points_count > 0) {
        ...
        LOG(INFO) << "HierarchicalClusterIndex set pq_train_points_count to "
                  << _conf.pq_train_points_count;
    }

    if (_conf.total_point_count < (uint32_t)FLAGS_train_points_count) {
        gflags::SetCommandLineOption("train_points_count",
                                     std::to_string(_conf.total_point_count).c_str());
    }
    ...
    int train_points_count = random_sampling(_conf.feature_file_name,
                                             _conf.total_point_count,
                                             FLAGS_train_points_count,
                                             _conf.feature_dim,
    ...
    if (train_points_count < (int)FLAGS_train_points_count) {
        gflags::SetCommandLineOption("train_points_count",
                                     std::to_string(train_points_count).c_str());
...
int
HierarchicalClusterIndex::init_single_build() {
    TIME_FUNCTION();
...
int
HierarchicalClusterIndex::single_build(BuildInfo* build_info) {
    if (_model == nullptr) {
    ...
    for (size_t i = 0; i < _conf.coarse_cluster_count; ++i) {
        points_coarse_terms[i] = cblas_sdot(
            _conf.feature_dim, chunk_points, 1, _coarse_vocab + i * _conf.feature_dim, 1);
    }
    for (size_t i = 0; i < _conf.fine_cluster_count; ++i) {
        points_fine_terms[i] =
            cblas_sdot(_conf.feature_dim, chunk_points, 1, _fine_vocab + i * _conf.feature_dim, 1);
    }

    float point_norms = cblas_sdot(_conf.feature_dim, chunk_points, 1, chunk_points, 1) / 2;
    nearest_cell_assign(points_coarse_terms.get(),
                        points_fine_terms.get(),
                        point_norms,
                        build_info->nearest_cell[0],
                        0);
    ...
    if (_conf.spilled_copy_num == 1) {
        bak_cell_assign2(points_coarse_terms.get(),
                         points_fine_terms.get(),
                         point_norms,
...
void
HierarchicalClusterIndex::init_context_pool() {
    //初始化cpu逻辑内核数个context

    LOG(INFO) << "init_context_pool: " << FLAGS_context_initial_pool_size;
    ...
    // disable when search (tasks nums is small, disable to avoid dynamic decrease thread nums)
    omp_set_dynamic(false);
}

const float*
HierarchicalClusterIndex::normalization(SearchContext* context, const float* feature) {
    SearchCellData& search_cell_data = context->get_search_cell_data();
...
int
getFileLineCnt(const char* fileName) {
    struct stat st;
